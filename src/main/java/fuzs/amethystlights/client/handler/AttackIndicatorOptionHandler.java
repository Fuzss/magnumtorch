...
public class AttackIndicatorOptionHandler {
    private static final Component ATTACK_INDICATOR_TOOLTIP_COMPONENT = new TextComponent(String.format("Attack Indicator has been disabled by %s mod.", AmethystLights.MOD_NAME));

    @SubscribeEvent
    public void onScreenInit(final ScreenEvent.InitScreenEvent.Post evt) {
        if (AmethystLights.CONFIG.server().torch.removeCooldown && evt.getScreen() instanceof VideoSettingsScreen) {
