package fuzs.amethystlights.client.handler;

import fuzs.amethystlights.AmethystLights;
import fuzs.amethystlights.mixin.accessor.LivingEntityAccessor;
import fuzs.amethystlights.mixin.client.accessor.MinecraftAccessor;
import fuzs.amethystlights.mixin.client.accessor.MultiPlayerGameModeAccessor;
import fuzs.amethystlights.network.client.C2SSweepAttackMessage;
import fuzs.amethystlights.network.client.C2SSwingArmMessage;
...
public class AttackAirHandler {
    ...
    private void resetMissTime() {
        if (!AmethystLights.CONFIG.server().combatTests.holdAttackButton) return;
        final Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.gameMode.hasMissTime()) {
            ((MinecraftAccessor) minecraft).setMissTime(AmethystLights.CONFIG.server().combatTests.holdAttackButtonDelay);
        }
    }

    @SubscribeEvent
    public void onLeftClickEmpty$airAttack(final PlayerInteractEvent.LeftClickEmpty evt) {
        if (!AmethystLights.CONFIG.server().combatTests.airSweepAttack) return;
        if (AmethystLights.CONFIG.server().combatTests.continuousAirSweeping || this.airSweepTime <= 0) {
            final Minecraft minecraft = Minecraft.getInstance();
            ((MultiPlayerGameModeAccessor) minecraft.gameMode).callEnsureHasSentCarriedItem();
            AmethystLights.NETWORK.sendToServer(new C2SSweepAttackMessage((minecraft.player).isShiftKeyDown()));
        }
        // one more than auto-attacking to prevent both from working together as it would be too op
        this.airSweepTime = AmethystLights.CONFIG.server().combatTests.holdAttackButtonDelay + 1;
    }
    ...
    public void onClickInput$minAttackStrength(InputEvent.ClickInputEvent evt) {
        ...
        if (evt.isAttack() && minecraft.hitResult.getType() != HitResult.Type.BLOCK) {
            ...
            if (minecraft.player.getAttackStrengthScale(0.5F) < AmethystLights.CONFIG.server().combatTests.minAttackStrength) {
    ...
    public void onClickInput$retainEnergy(InputEvent.ClickInputEvent evt) {
        if (!AmethystLights.CONFIG.server().combatTests.retainEnergy) return;
        final Minecraft minecraft = Minecraft.getInstance();
        if (evt.isAttack() && minecraft.hitResult.getType() == HitResult.Type.MISS) {
            ...
            if (evt.shouldSwingHand()) {
                ...
                AmethystLights.NETWORK.sendToServer(new C2SSwingArmMessage(evt.getHand()));
            }
        }
    }
